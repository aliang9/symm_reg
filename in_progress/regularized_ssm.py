...
class RegularizedSSM(LowRankNonlinearStateSpaceModel):
    ...
    def __init__(self, 
        ...
        """
        super().__init__(dynamics_mod, likelihood_pdf, initial_c_pdf,
                        backward_encoder, local_encoder, nl_filter, device)
        ...
        self.lie_regularizer = LieDerivativeRegularizer(
            self.dynamics_mod.mean_fn, 
            target_vector_field,
            normalize=lie_normalize
        )
    ...
    def forward(self, 
                ...
                p_mask_b: float = 0.0,
                get_P_s: bool = False,
                **kwargs) -> tuple[torch.Tensor, torch.Tensor, Dict[str, Any]]:
        """
        ...
        # ELBO loss (negative ELBO, so we minimize)
        elbo_loss = stats['kl'] - ell
        elbo_loss = elbo_loss.sum(dim=-1).mean()
    ...
    def _compute_regularization_losses(self, z_s: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
    ...
    def evaluate_regularizers_at_points(self, 
        ...
        if self.lambda_curvature > 0 and self.curvature_regularizer is not None:
            results['curvature_values'] = self.curvature_regularizer.eval_regularizer(points)
            
        return results


class RegularizedLightningSSM:
    """
    ...
    def __init__(self, regularized_ssm: RegularizedSSM, cfg):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if stats['curvature_loss'] > 0:
            self.log('valid_curvature', stats['curvature_loss'], prog_bar=False, sync_dist=True)
