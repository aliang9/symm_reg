...
def update_bias_to_match_target_rate(
    current_rate: np.ndarray, current_bias: np.ndarray, target_rate: float
) -> np.ndarray:
...
def compute_snr(
    latent_traj: np.ndarray, C: np.ndarray, b: np.ndarray, target_rate: float
...
def generate_poisson_observations(
    trajectories: np.ndarray, settings: ObservationSettings
    ...
    if settings.snr_target is not None:
        ...
        if current_snr > 0:  # Only scale if SNR is positive
            ...
            C = C * gain
            firing_rates = compute_firing_rate(latent_traj, C, b)
            current_snr, b = compute_snr(
                latent_traj, C, b, settings.target_rate_per_bin
            )
        else:
            current_snr = 0.0
    else:
...
def generate_observations(
    ...
    """Generate noisy observations from trajectories using specified observation ode."""
    if settings.model == ObservationModel.GAUSSIAN:
        ...
        for i in range(n_trajectories):
            for t in range(n_timesteps):
                clean_observations[i, t] = (
                    settings.observation_matrix @ trajectories[i, t]
                )
        ...
        observations, C, b, firing_rates, snr = generate_poisson_observations(
...
def main(return_data=False):
    ...
    # Create random observation matrix (neurons y latents)
    ...
    gaussian_observations, gaussian_snr = generate_observations(
        trajectories, gaussian_settings
    )
    ...
    poisson_observations, poisson_snr = generate_observations(
        trajectories, poisson_settings
    )
    ...
    _, _, _, firing_rates, _ = generate_poisson_observations(
        trajectories, poisson_settings
    )

    # Plot Poisson analysis
    poisson_fig = plot_poisson_analysis(
        trajectories, poisson_observations, firing_rates, poisson_settings
    )
    ...
    print(
        f"Mean angular velocity: {metrics['mean_angular_velocity']:.3f} ± {metrics['std_angular_velocity']:.3f}"
    )
    ...
    print(
        f"Mean absolute observation error: {gaussian_mean_error:.3f} ± {gaussian_std_error:.3f}"
    )
    ...
    if return_data:
        return {
            ...
            "inv_man": inv_man,
            "vector_field": {"X": X, "y": Y, "U": U_pert, "V": V_pert},
        }
